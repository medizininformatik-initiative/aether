...
// BatchStage is one stage of the TORCH batch pipeline.
type BatchStage string

const (
	StageConsentFetch     BatchStage = "CONSENT_FETCH"
	StageDirectLoad       BatchStage = "DIRECT_LOAD"
	StageReferenceResolve BatchStage = "REFERENCE_RESOLVE"
	StageCascadingDelete  BatchStage = "CASCADING_DELETE"
	StageCopyRedact       BatchStage = "COPY_REDACT"
)
...
// TORCHBatchStages is the TORCH batch pipeline in order. TORCH processes each
// batch through these stages.
var TORCHBatchStages = []BatchStage{
	StageConsentFetch,
	StageDirectLoad,
	StageReferenceResolve,
	StageCascadingDelete,
	StageCopyRedact,
}
...
func (b TORCHActiveBatch) StageIndex() int {
	for i, stage := range TORCHBatchStages {
		if stage == b.Stage {
			return i + 1
		}
	}
...
func (p TORCHProgress) Summary() string {
	...
	for i, batch := range p.ActiveBatches {
		stages[i] = fmt.Sprintf("%s (%d/%d)", batch.Stage, batch.StageIndex(), len(TORCHBatchStages))
	}
...
// ProgressReporter is the seam an Extractor implements when it can report
// extraction progress while it polls.
type ProgressReporter interface {
	SetProgressHandler(func(TORCHProgress))
}
...
func activeBatchFromExtension(ext fhirExtension) TORCHActiveBatch {
	...
	for _, sub := range ext.Extension {
		...
		case "stage":
			batch.Stage = BatchStage(sub.ValueString)
		}
	}
	return batch
}
