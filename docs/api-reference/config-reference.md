...
services:
  ...
  dimp:
    ...
    timeout: duration                    # default: PT30S
    auth:
      username: string
      password: string
      ...
      api_key: string
      api_key_header: string             # default: x-api-key
    anonymization_config: string         # sent with each $de-identify request
...
| `anonymization_config` | string | - | Path to the anonymization YAML file. Aether sends it with each `$de-identify` request, so the service does not need a restart for configuration changes. This needs FHIR-Pseudonymizer v2.34.0 or later. Without a path, the service uses its own anonymization file. The `pipeline start --anonymization-config` flag overrides this key. Set `parameters.keyDerivationContext` in this file to derive a project key from a master key on the server. See [Key Derivation](../guides/dimp.md#key-derivation). |
